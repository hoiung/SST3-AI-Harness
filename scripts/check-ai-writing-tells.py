#!/usr/bin/env python3
"""
Marker-driven voice guard for operator-voice content.
...
regardless of iamhoi markers, because 100% of their content is operator-voice
...
# Whole-file scan: files where 100% of content is operator-voice, recruiter-
...
# (the operator's pre-AI writing). Treating it as exempt prevents the iamhoi region-tag
...
def is_whitelisted(file_path: Path, repo_root: Path) -> bool:
    """Whole-file scan whitelist — 100% operator-voice recruiter-facing files."""
    rel = str(file_path.relative_to(repo_root))
...
def scan_file(file_path: Path, repo_root: Path) -> list[Finding]:
    ...
    scanning: the whole file is operator-voice content, so the whole file is
