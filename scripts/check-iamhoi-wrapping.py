#!/usr/bin/env python3
"""Enforce iamhoi-marker wrapping on new operator-voice posts.

Complements `check-ai-writing-tells.py` (which scans INSIDE iamhoi regions)
by ENFORCING that posts containing first-person operator-voice prose actually
HAVE the wrapping markers in the first place. Without this gate, an author
can write a full operator-voice post and the voice guard silently skips it
...
First-person detection:
  ...
  - KEEP_LIST vocabulary signal removed after Issue #3 Sonnet review surfaced
    ...
    not being operator-voice. the operator's prose is reliably first-person; KEEP_LIST
...
def check_file(path: Path, check_only_new: bool) -> tuple[bool, str]:
    ...
    if has_voice_prose(body):
        return False, (
            f"ERR: {path}: post contains operator-voice prose but no iamhoi wrapping. "
...
def main() -> int:
    ...
    if failures:
        ...
        print(
            ...
            f"See ~/DevProjects/voice-doc-repo/cv-linkedin/VOICE_PROFILE.md Section 8 + AP #15.",
            file=sys.stderr,
        )
        return 1
