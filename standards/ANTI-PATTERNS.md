...
| **Homelab-bootstrap** (single-node NUC provisioning, operator-initiated from master) | `lab-ops/tests/phase5-gate.ps1` (admin pwsh, on-NUC) — 21-23 named hardware-gate + deliverable + push-model invariants A1-I9 (post dotfiles#479: `phase5-gate.ps1` lives in lab-ops; master-side `verify-node.sh` invokes it cross-repo) | Master-side `dotfiles/scripts/verify-node.sh --role lab --gh-auth --windows-tasks` exit 0 + `ssh HU-<MODEL>` (port 22) + `ssh HU-<MODEL>-wsl` (port 2222) both reachable + `lab-ops/tests/lab-bound-bw-grep-gate.sh` exit 0 (no `bw_get`/`Get-BitwardenSecret`/`Bootstrap-BitwardenSession` reaches lab-bound code paths) | n/a — exercise every invariant the change touched (per-group table in `lab-ops/docs/runbooks/homelab.md` "phase5-gate invariant catalog") | `lab-ops/scripts/bootstrap-lab-node.ps1` / `dotfiles/scripts/provision.sh` (STAYS — master-side bootstrap orchestration) / `dotfiles/scripts/secrets-pull.sh` (STAYS — not lab-bound) / `dotfiles/scripts/verify-node.sh` (STAYS per dotfiles#479 G3 STAY-CROSS-REPO) / `lab-ops/windows/wsl-service/setup-wsl-service.ps1` / `lab-ops/windows/wsl-service/setup-portproxy.ps1` / `lab-ops/scripts/refresh-usb-kit.sh` / `lab-ops/scripts/push-post-bootstrap-secrets.sh` / `lab-ops/scripts/configure-auto-login.ps1` / `lab-ops/scripts/utils/USB-Helpers.ps1` / `lab-ops/scripts/utils/Bitwarden-Helpers.ps1` (master-only) / `lab-ops/scripts/utils/bw-helpers.sh` (master-only) / `dotfiles/wsl/packages.txt` (STAYS) / `lab-ops/docs/runbooks/` (#474 rename: `bootstrap-nuc.ps1` → `bootstrap-lab-node.ps1`; #475 push-model: `bw` master-only, secrets reach lab via Channel 1 USB-bake + Channel 2 master-push-via-Tailscale-SSH; #479 migration: lab-automation surface extracted to `hoiung/lab-ops` consumer repo, master-side staying scripts cross-repo source `lab-ops/scripts/utils/push-helpers.sh`) | Wired-only convention (WiFi disabled, reservation against Ethernet MAC); never schedule destructive `wsl --shutdown` test while operator session active; G14 Modern Standby OFF on laptops; lab-role only for auto-login; **NO `bw` on lab** (asserted by G17 + H9-H12: `bw` absent + grep-gate clean + USB-wiped post-consume + no-DPAPI Bitwarden artefact); G16 BATACTIONCRIT/HiberbootEnabled gated `$Role -ne master`; I8 Tailnet identity + tag; I9 no plaintext DefaultPassword leak; extended I6 `gh auth status` content-check requires `repo + workflow + read:org` scopes |
...
- (a) **pure git subcommands**: use `git -C <path> <subcmd>` (no CWD change at all). MANDATORY for any cross-repo git operation. Examples: `git -C ../SST3-AI-Harness status`, `git -C ~/DevProjects/blog-priv log --oneline`.
