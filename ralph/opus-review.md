...
Preconditions (code-touching PRs, run once): `bash dotfiles/scripts/sst3-graph-status.sh` returns `total_nodes > 0` AND `last_updated < 24h`. If either fails, skip to fallback clause below.

- [ ] Dead code detection: `bash dotfiles/scripts/sst3-graph-large.sh 200 <lang>` + manual orphan scan. For each candidate: `bash dotfiles/scripts/sst3-graph-callers.sh <name> <lang>` returns empty in the same module ⇒ orphan. Subagent confirms whether call is via reflection / dynamic dispatch (not an orphan) or a true orphan (cleanup target).
- [ ] Impact scope validation: `bash dotfiles/scripts/sst3-graph-impact.sh <base-branch>` — enumerate all impacted modules; identify unexpected cross-boundary edges. Document each boundary: intended (defence-in-depth / architectural layering) vs emergent (refactor target). (Note: Phase A wrapper-lane does not expose `max_depth` parameter; deeper-than-1-hop scope requires subagent walk.)
- [ ] Large functions audit: confirm no function in diff exceeded 200 lines (`bash dotfiles/scripts/sst3-graph-large.sh 200 <lang>`, scoped via subagent grep on diff files for `file_path_pattern` equivalent). If any did, architectural red flag — require refactor.
