...
- [ ] **Governance drift audit — two-tier cadence rule**: pull the issue-body PATCH event log via `mcp__github-checkbox__get_issue_events` (ToolSearch-bootstrap if deferred per `../standards/STANDARDS.md` "MCP Tool Schema Loading") and the solo-branch git log. Classify every checked `[x]` box into one of two tiers:
  - **Tier A — Phase-deliverable checkboxes** (items in Acceptance Criteria Phases 1..N that describe a concrete deliverable — a file edit, commit, function, section, table, example): **STRICT interleaving required**. PATCH events MUST fall within the same phase's commit window (between first commit of phase N and first commit of phase N+1). Cluster-at-end = FAIL.
...
- [ ] **Rule**: **Flag Tier A batching as AP #20 drift.** **Do NOT flag Tier B batching as drift.** A Tier-B-only batch is acceptable and expected for honest post-condition evaluation. Canonical failure mode: `../standards/ANTI-PATTERNS.md` **AP #20**; canonical evidence patterns: `../reference/tool-selection-guide.md` Example 2.
