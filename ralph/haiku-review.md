...
- [ ] **Doc-lane (diff-triggered, #484 W6.3 — runs whenever the diff touches `*.md`/frontmatter, regardless of `graph_applicable` / doc-only-exemption)**: run `bash dotfiles/scripts/sst3-doc-lint.sh <changed.md...>` + `bash dotfiles/scripts/sst3-doc-links.sh <changed.md...>` on the diff's changed docs. **Both are triage REPORTERS, not zero-gates** — they emit one NDJSON object per finding (the repo has no `.markdownlint*` config, so canonical docs carry hundreds of default MD013/MD060 findings — e.g. STANDARDS.md ≈548; that pre-existing baseline is NOT this gate's concern). **Run-proof caveat (#484 Stage-5 — doc-lint exit code is not a run-proof)**: exit 0 alone does NOT prove the engine ran. `sst3-doc-links.sh` always emits a `sst3-doc-links: scanned N path(s)` stderr sentinel (its absence ⇒ it did not run), but `sst3-doc-lint.sh` has no equivalent always-on sentinel and its `done || true` swallows a mid-run markdownlint crash — so for the LINT half confirm it ran by the expected NDJSON volume: any real `*.md` diff yields the hundreds-of-MD-findings baseline; **empty `sst3-doc-lint.sh` output on a non-trivial `*.md` diff means the engine did NOT run — investigate, do not record as clean**. (Adding a `scanned N path(s)` sentinel to `sst3-doc-lint.sh` for parity with its sibling is tracked as Cross-Issue-Impact on its own `solo/wrapper-fix-*` branch + new frozen self-test fixture per the wrapper-lane self-test discipline — AP #21, not autonomously filed.) Triage rule: a finding is a FAIL only if it is **net-new on a doc this diff changed** (a dead/borked link the diff introduced, or a markdown-structure break the diff added) — fix it or justify it in RESULT. Record the run + the net-new-finding triage in RESULT. Skip-clean only if the diff touches no doc file.
