...
- [ ] **Doc-lane (diff-triggered, #484 W6.3 — runs whenever the diff touches `*.md`/frontmatter, regardless of `graph_applicable` / doc-only-exemption)**: `bash dotfiles/scripts/sst3-doc-lint.sh <changed.md...>` exit 0 (markdownlint clean) + `bash dotfiles/scripts/sst3-doc-links.sh <changed.md...>` exit 0 (no dead links). Record exit codes in RESULT. Skip-clean only if the diff touches no doc file.
